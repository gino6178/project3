...
class MPM_Simulator_WARP:
    ...
    def set_material_from_torch(self, E=None, nu=None, density=None,
                                yield_stress=None, device="cuda:0"):
        """Give every particle its own material instead of one value for the whole body.

        The fields are already per particle -- `E`, `nu`, `yield_stress` and
        `particle_density` are all allocated at `n_particles`, and `set_parameters_dict`
        only fills them with a constant. A lattice knows which cells are skin and which are
        interior, and has a colour per cell, so it can say where the peel is; nothing about
        the solver needed changing to use that.

        Mass is density times volume and is consumed by the transfer kernels, so it is
        recomputed here rather than left at whatever the constant pass wrote.
        """
        import torch as _t
        from mpm_solver_warp.warp_utils import torch2warp_float

        def _fill(field, t):
            v = _t.as_tensor(t, dtype=_t.float32, device=device).contiguous().reshape(-1)
            assert v.shape[0] == self.n_particles, (v.shape[0], self.n_particles)
            wp.copy(field, torch2warp_float(v, dvc=device))

        if E is not None:
            _fill(self.mpm_model.E, E)
        if nu is not None:
            _fill(self.mpm_model.nu, nu)
        if yield_stress is not None:
            _fill(self.mpm_model.yield_stress, yield_stress)
        if density is not None:
            _fill(self.mpm_state.particle_density, density)
            wp.launch(
