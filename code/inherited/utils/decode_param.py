...
def decode_param_json(json_file):
    ...
    # Publish the object's in-plane axis so every camera built from this config shares one
    # frame with the lattice and the initialisation, rather than each deriving its own from a
    # constant. Absent, this is None and the frame is what it always was.
    from utils import camera_view_utils as _cvu
    _cvu.INPLANE_REF = sim_params.get("mpm_space_inplane_axis")
