...
def generate_camera_rotation_matrix(camera_to_object, object_vertical_downward,
                                    roll_deg=0.0):
    """Orientation for a camera at `camera_to_object`, held upright against the object.

    Looking straight down the object's own vertical axis leaves the upright direction
    undefined, and the caller asks for exactly that: every transverse cut is rendered at
    elevation 90, and the six-direction projection uses the two poles. There
    `object_vertical_downward` is the view direction, the component perpendicular to it is
    zero, and normalising it divides by the floating-point residue -- so the in-plane axes
    come out of rounding noise. Usually that is merely an arbitrary roll, which a round
    fruit does not show; sometimes the residue is small enough that the cross below loses
    orthogonality and the whole matrix is wrong, which is the released watermelon framed
    from inside itself while its longitudinal cuts were clean. Silent either way.

    So detect it and pick a definite axis instead: the world axis furthest from the view
    direction, projected perpendicular to it. Which one it is only sets the roll, which is
    what was undefined to begin with; that it is stable is the point.
    """
    ...
    if np.linalg.norm(camera_y) < 1e-6:
        alt = np.zeros(3)
        alt[int(np.argmin(np.abs(camera_to_object)))] = 1.0
        camera_y = alt - np.dot(alt, camera_to_object) * camera_to_object
    camera_y = camera_y / np.linalg.norm(camera_y)  # second column
    if roll_deg:
        # Turn the camera about its own view axis. Looking down the object's vertical axis, the
        # upright direction is not determined by the scene, and neither is it determined by a
        # reference photograph of a different specimen -- that orange's segments are at their
        # own angles, not ours. Fixing a roll makes the model commit to those angles; drawing
        # one per iteration marginalises over them, and what survives supervision is the part
        # of the section that does not depend on the phase.
        t = np.deg2rad(roll_deg)
        camera_y = (camera_y * np.cos(t)
                    + np.cross(camera_to_object, camera_y) * np.sin(t))
...
# Where azimuth zero points, in the plane perpendicular to the vertical axis. It used to be
# the projection of [1, 1, 1] and nothing else, which is an arbitrary direction: for a loaf,
# whose vertical is world -x, it lands on [0, 1, 1]/sqrt(2) -- exactly 45 degrees from the
# object's own in-plane axes, so its square cross-section rendered as a diamond and its
# interior, which is laid out on those axes, came out turned against its own shell. A round
# object cannot show this, which is why three of them did not.
#
# Set from the physics config by decode_param_json when it carries mpm_space_inplane_axis, so
# no camera call site changes and an object without the field behaves exactly as before.
INPLANE_REF = None


def generate_local_coord(vertical_vector, horizontal_ref=None):
    vertical_vector = vertical_vector / np.linalg.norm(vertical_vector)
    horizontal_1 = np.array([1, 1, 1]) if horizontal_ref is None \
        else np.asarray(horizontal_ref, dtype=np.float64)
    horizontal_1 = horizontal_1 / max(np.linalg.norm(horizontal_1), 1e-12)
    # Gram-Schmidt degenerates when the reference is *parallel* to the vertical, not when it
    # is perpendicular. The test used to read `< 0.01`, which is the perpendicular case: with
    # the released [1, 1, 1] it almost never fired, but an in-plane axis is perpendicular by
    # definition, so every one supplied was silently swapped for the same fallback and three
    # different axes produced identical frames.
    if np.abs(np.dot(horizontal_1, vertical_vector)) > 0.99:
...
def get_camera_position_and_rotation(
    azimuth, elevation, radius, view_center, observant_coordinates, roll_deg=0.0
):
    # get camera position
    ...
    R = generate_camera_rotation_matrix(
        view_center - position, -observant_coordinates[:, 2], roll_deg=roll_deg
    )
    return position, R
...
def get_camera_view(
    ...
    `roll_deg` turns the camera about its own view axis. See
    `generate_camera_rotation_matrix`: at the pole the upright direction is undetermined, and
    for a transverse section supervised by a photograph of another specimen it stays
    undetermined, so the caller may want to draw it rather than fix it.
    """
