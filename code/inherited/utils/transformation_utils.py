...
def get_center_view_worldspace_and_observant_coordinate(
    ...
    viewpoint_center_worldspace_t = viewpoint_center_worldspace
    viewpoint_center_worldspace = np.squeeze(
        viewpoint_center_worldspace.clone().detach().cpu().numpy(), 0
    )
    from utils import camera_view_utils as _cvu
    href = None
    if _cvu.INPLANE_REF is not None:
        _ip = torch.tensor(_cvu.INPLANE_REF, dtype=mpm_space_viewpoint_center.dtype,
                           device=mpm_space_viewpoint_center.device).reshape(1, 3)
        _ipw = undo_all_transforms(_ip + mpm_space_viewpoint_center, rotation_matrices,
                                   scale_origin, original_mean_pos)
        href = np.squeeze((_ipw - viewpoint_center_worldspace_t
                           ).clone().detach().cpu().numpy(), 0)
    vertical, h1, h2 = generate_local_coord(
        np.squeeze(world_space_vertical_axis.clone().detach().cpu().numpy(), 0), href
    )
