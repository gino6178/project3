...
# One path, any object.
#
#   bash repro/run.sh orange
#   bash repro/run.sh watermelon
#   bash repro/run.sh doughnut
#   GPU=1 bash method/run.sh orange        # on the second card
#   bash repro/run.sh orange eval         # just re-score an existing model
#
...
source "$HERE/stages.sh"

OBJ=${1:?usage: run.sh OBJECT [stage]}
ONLY=${2:-all}
CONF="$HERE/objects/$OBJ.conf"
[ -f "$CONF" ] || { echo "no such object: $OBJ  (have: $(ls "$HERE/objects" | sed 's/.conf$//' | tr '\n' ' '))"; exit 1; }
