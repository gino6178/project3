...
if [ ! -f "$W/cams_$OBJ.npz" ]; then
  ...
  CUDA_VISIBLE_DEVICES=$GPU LAT=$LATD/gs_fill.ply CFG=$CFG OUT=$W/cams_$OBJ.npz \
...
exec env CUDA_VISIBLE_DEVICES=$GPU ROUTE=1 SHELL_PIN=1 EXT_VIEWS=0 VOXEL_SMOOTH=1 \
  STATE=$W/state_$OBJ.pt CAMS=$W/cams_$OBJ.npz REF_H=$REF_H REF_V=$REF_V \
