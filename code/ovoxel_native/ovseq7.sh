...
#   q8     the same, plus SEC_SCHED=cycle                          planes walked by the formula
set -u
W=/workspace/ovoxel_native
OBJS="orange_sp watermelon_sp apple1_sp bread_sp cake2_sp pomegranate2_sp doughnut"
GPUS="0 1 2 3"

rm -rf "$W/queue8"; mkdir -p "$W/queue8"; : > "$W/ovseq7_status.txt"
i=0
for o in $OBJS; do
  printf "%s q8" "$o" > "$W/queue8/$(printf %02d $i).job"; i=$((i+1))
done

worker () {
  local g=$1 j o tag
  while :; do
    j=$(ls "$W"/queue8/*.job 2>/dev/null | head -1)
    [ -n "$j" ] || break
    mv "$j" "$j.taken" 2>/dev/null || continue
    read -r o tag < "$j.taken"
    TAG=q8 bash "$W/objrun.sh" "$o" "$g" ITERS=325 SEC_TV=0.1 SEC_TV_PERP=4 REF_SAMPLE=1 \
      SEC_SCHED=cycle > "$W/q8_$o.log" 2>&1
    printf "%-18s %s gpu %s exit %s\n" "$o" "$tag" "$g" "$?" >> "$W/ovseq7_status.txt"
  done
}

for g in $GPUS; do worker "$g" & done
wait
echo SEQ7_DONE >> "$W/ovseq7_status.txt"
