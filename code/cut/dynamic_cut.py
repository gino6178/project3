...
"""
import sys, os
# The root is where the object's configuration and lattice live, and it is not this file's
# parent on every machine that runs it.
_FN = os.environ.get("FN_ROOT", "/home/gino/project/FruitNinja_clean")
sys.path.append(_FN)
sys.path.append(os.environ.get("GS_ROOT", _FN + "/gaussian-splatting"))
os.chdir(_FN)
...
def main(ply, outdir, frames=210, flag_path=None, view_az=105, substeps=30, dt=3e-4):
    ...
    if _preset:
        from material_field import material_field, summarise, _rgb
        _lvl = None
        ...
        if _preset == "classes" and _lbl and os.path.exists(_lbl):
            from material_field import material_from_labels
