...
def _photo(spec):
    ...
    # REF_RANDOM_ASSIGN draws the photograph afresh each time a target is built, so a plane sees a
    # different member of its family from one iteration to the next. The default is the fixed map
    # above: a plane keeps one photograph for the whole run, which is what makes the conflict on a
    # shared cell the same conflict on every pass.
    if os.environ.get("REF_RANDOM_ASSIGN", "0") == "1":
        import random as _rr
        k = _rr.randrange(len(files))
    key = (spec, k)
...
def one_step_sds_orange(image, depth, total_epochs, pipe, view_cut):
    ...
    if _pdir:
        ...
        # Phases solved against the cross-family objective rather than within the family. The
        # transverse and longitudinal planes meet in known lines, so the disagreement between the
        # two families is a function of the phases alone and can be minimised before training.
        # See phaseopt.py; the file holds one angle per transverse photograph, in radians.
        _po = os.environ.get("REF_PHASE_SOLVED", "")
        if _po and view_cut == "horizontal" and os.path.exists(_po):
            import numpy as _np
            _ang = _np.load(_po)
            _files = _photo_files(_pdir) if "_photo_files" in globals() else None
            _k = _PLANE["idx"] * len(_ang) // max(_PLANE["n"], 1) % len(_ang)
            _p = _photo(_pdir).rotate(-float(_np.degrees(_ang[_k])), resample=Image.BICUBIC,
                                      fillcolor=(255, 255, 255))
        elif REF_PHASE_MODE and view_cut == "horizontal":
            _p = (_shared_phase(_pdir) if REF_PHASE_MODE == "share"
                  else _phase_aligned(_pdir))
        else:
            _p = _photo(_pdir)
        # Pose noise, for the ablation that asks how much misalignment the lattice absorbs.
        # A reference is content, not geometry: the plane is ours and _fit_disc warps the image
        # onto the silhouette that plane renders, so there is no pose to estimate and none to get
        # wrong. What a rotation does instead is put the photograph's angular structure at an
        # angle the other references do not share, which is the failure this is meant to provoke.
        _rot = float(os.environ.get("REF_POSE_NOISE", "0"))
        if _rot > 0:
            import random as _rnd
            _g = _rnd.Random(hash((_pdir, view_cut, _PLANE["idx"])) & 0xffffffff)
            _p = _p.rotate(_g.uniform(-_rot, _rot), resample=Image.BICUBIC,
                           fillcolor=(255, 255, 255))
