...
def main(ply, cfg, demo, out_dir, n=12, size=512, seed=7):
    ...
    for i in range(n // 2):
        ...
        #
        # This band is NOT a region training never reaches, and an earlier version of this comment
        # said it was. The walk takes H_STEPS=24 steps and H_LO:H_HI supervises centres 4..19; at
        # JITTER=0.5 each slot's window is exactly one step wide, so the windows tile with no gap
        # and training covers f in [0.146, 0.813] continuously. What is held out is the particular
        # plane, not the depth. The only genuinely unseen quantity in this project is the
        # left-out photograph of the leave-one-out folds.
