...
thing read is the model and its physics/demo configs.
    ...
    python method/common/eval/exterior_views.py MODEL CFG DEMO OUT.png [size] [OVOX.npz]

Given an O-Voxel npz the same six views are drawn off the dual-grid surface instead, which is
the exterior with no Gaussians anywhere in it.
"""
import os as _os
...
def main(model, cfg, demo, out_png, size=512, ovox=None):
    ...
    surf = None
    if ovox:
        from method.common.eval.ovox_cuts import sim_apply, sim_fit, sim_inv, _splat
        z = np.load(ovox)
        V = z["mesh_v"].astype(np.float64)
        F = z["mesh_f"].astype(np.int64) if "mesh_f" in z.files else None
        rgb = z["rgb"].astype(np.float32)
        if F is not None:                       # points on the triangles, not only at the corners
            k = 3
            t = (np.arange(k) + 0.5) / k
            ...
            pts, cols = V, rgb
        # the npz is in the ply's own frame and the cameras project in the renderer's; both are
        # row-aligned through the ply, so the map is fitted rather than unpicked
        xyz = np.stack([PlyData.read(ply).elements[0][a] for a in "xyz"], 1).astype(np.float64)
        W = world.detach().cpu().numpy().astype(np.float64)
        f = sim_inv(sim_fit(W, xyz[:len(W)]))
        surf = (sim_apply(pts, f), cols)
        print(f"  O-Voxel surface: {len(pts):,} samples, frame residual {f[3]:.2e}")
...
if __name__ == "__main__":
    main(sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4],
         ...
         ovox=sys.argv[6] if len(sys.argv) > 6 else None)
