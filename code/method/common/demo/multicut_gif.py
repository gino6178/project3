...
def faces(allf, pid, K, k_samp=3):
    ...
    Returns the points, the piece each belongs to, the face normal, and whether the face was
    already on the outside of the object.
    """
    occ = Occ(allf)
    ...
    for d, corners in FACE:
        ...
        CTR.append(np.repeat(ctr, len(ga), axis=0))
    return (np.concatenate(P), np.concatenate(W), np.concatenate(N),
            np.concatenate(EXT), np.concatenate(CTR))
...
def main(lattice_dir, ply, cfg, demo, out_gif, n_frames=72, size=440, ss=2, k_samp=3,
    ...
    pts, who, nrm, ext, fctr = faces(allf, pid, K, k_samp=k_samp)
    ...
    pts = pts * hf + org
    fctr = fctr * hf + org
    col = rgb[cKDTree(xyz).query(fctr, k=1)[1]]
    ...
    for i in range(n_frames):
        ...
        frames.append(np.asarray(gifcam.caption(img, lines, size=15, band=True)))
        if i % 12 == 0:
