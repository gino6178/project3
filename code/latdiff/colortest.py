"""Does SinDiffusion force its single training palette regardless of input colour?

Take one render, tint it several ways (hue/saturation shifts), SDEdit each, and compare output
colours. If the outputs all collapse to the same palette, the model is a colour-normaliser -- which
is why dense targets, all forced to one palette, cannot represent per-depth colour variation.
"""
import os, sys
...
@torch.no_grad()
def sdedit(x0,mask,s):
    ts=int(s*diff.num_timesteps)-1
...
def lab(t,wd):
    ...
    dr_.text((6,3),t,fill=(30,30,30),font=f);return np.asarray(im)
P,M=C["v_planes"],C["v_mvp"]; k=len(P)//2
...
# four colour casts of the same render
casts={"orig":lambda x:x,
       "red":lambda x:x*torch.tensor([1.3,0.7,0.7],device=dev).view(1,3,1,1),
       "green":lambda x:x*torch.tensor([0.7,1.3,0.7],device=dev).view(1,3,1,1),
       "blue":lambda x:x*torch.tensor([0.7,0.7,1.6],device=dev).view(1,3,1,1)}
rows=[]
for name,fn in casts.items():
    inp=(mask*fn(img[None])+(1-mask)*1).clamp(0,1)
    out=sdedit(inp*2-1,mask,0.4)*0.5+0.5
    # mean colour inside mask
    m=mask[0,0].bool(); mc=out[0][:,m].mean(1)
    rows.append((name,np.concatenate([arr(inp[0]),arr((mask*out+(1-mask))[0])],1),
                 f"in->out mean RGB: [{mc[0]:.2f},{mc[1]:.2f},{mc[2]:.2f}]"))
    print(f"  {name}: out mean RGB {[round(float(c),3) for c in mc]}",flush=True)
blocks=[]
for name,row,cap in rows:
    blocks+= [lab(f"{name} cast: input | SDEdit out    {cap}", row.shape[1]), row]
Image.fromarray(np.concatenate(blocks,0)).save("/workspace/colortest.png")
print("done")
