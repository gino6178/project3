...
TAG=os.environ.get("TAG","mt23"); NORMTV=float(os.environ.get("NORMTV","0.0")); NORMDD=float(os.environ.get("NORMDD","1.5")); LNORM=float(os.environ.get("LNORM","0.0")); LAP=float(os.environ.get("LAP","0.0")); LR=float(os.environ.get("LR","5e-3")); ANCHOR=float(os.environ.get("ANCHOR","1.0"))
...
_supmask=torch.zeros(len(_cen),device=dev)
for _t in T:                                   # every target plane, held and supervised
    _nn=_t["n"].to(dev).float()
    _supmask=torch.maximum(_supmask,((_cen@_nn+float(_t["d"])).abs()<hc_).float())
_w=(1.0-_supmask)   # per-cell Laplacian weight
print(f"3D Laplacian: {len(_EI)} edges, LAP={LAP}, exempt {int(_supmask.sum())} supervised cells")
