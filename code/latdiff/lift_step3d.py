...
TAG=os.environ.get("TAG","mt24"); NORMTV=float(os.environ.get("NORMTV","0.0")); NORMDD=float(os.environ.get("NORMDD","1.5")); LNORM=float(os.environ.get("LNORM","0.0")); LAP=float(os.environ.get("LAP","0.0")); BETA=float(os.environ.get("BETA","40.0")); SOFTSIG=float(os.environ.get("SOFTSIG","1.2")); LR=float(os.environ.get("LR","5e-3")); ANCHOR=float(os.environ.get("ANCHOR","1.0"))
...
# cells within ~1 cell of a supervised plane are exempt from the Laplacian: keep the anchors sharp,
# only smooth the unsupervised gap cells where the sweep grid actually lives
hc_=float(st["hc"]); org_=torch.as_tensor(st["org"],dtype=torch.float32,device=dev)
_cen=(st["solid"].to(dev).float()+0.5)*hc_+org_
# #3 soft-mask: smooth Gaussian falloff of the exemption from each target plane (no hard C2 jump)
_softsup=torch.zeros(len(_cen),device=dev)
for _t in T:
    _nn=_t["n"].to(dev).float(); _dist=_cen@_nn+float(_t["d"])
    _softsup=torch.maximum(_softsup, torch.exp(-(_dist**2)/(2*(SOFTSIG*hc_)**2)))
_wgap=(1.0-_softsup)
# #2 edge-aware: relax smoothing where the Stage-1 field already has strong gradients (core, peel)
with torch.no_grad():
    _d0=(rgb0[_EJ]-rgb0[_EI]).abs().mean(1)
    _g=torch.zeros(len(rgb0),device=dev); _c=torch.zeros(len(rgb0),device=dev); _o=torch.ones_like(_d0)
    _g.index_add_(0,_EI,_d0); _g.index_add_(0,_EJ,_d0); _c.index_add_(0,_EI,_o); _c.index_add_(0,_EJ,_o)
    _gmag=_g/_c.clamp_min(1); _wedge=1.0/(1.0+BETA*_gmag)
_w=_wgap*_wedge
print(f"3D Laplacian edge-aware+soft: {len(_EI)} edges, LAP={LAP} BETA={BETA} SOFTSIG={SOFTSIG}, mean w={float(_w.mean()):.3f}")
...
for step in range(1,STEPS+1):
    ...
    if LAP>0:
        ...
        lap=((L**2).mean(1)*_w).sum()/_w.sum().clamp_min(1)
