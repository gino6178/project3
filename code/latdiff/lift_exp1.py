...
TAG=os.environ.get("TAG","mt19"); NORMTV=float(os.environ.get("NORMTV","0.0")); NORMDD=float(os.environ.get("NORMDD","1.5")); LNORM=float(os.environ.get("LNORM","0.0")); JIT=float(os.environ.get("JIT","0.0")); JITANG=float(os.environ.get("JITANG","3.0")); LR=float(os.environ.get("LR","5e-3")); ANCHOR=float(os.environ.get("ANCHOR","1.0"))
...
T=torch.load(f"{W}/targets_exp1_{OBJ}.pt", map_location=dev)
for t in T:
...
for step in range(1,STEPS+1):
    ...
    import math
    for t in T:
        if JIT>0 and t["name"].endswith("_h"):
            nj=t["n"]
            if JITANG>0:                                    # random small tilt of the plane normal
                g=torch.randn(3,device=dev); g=g-(g@nj)*nj
                nj=torch.nn.functional.normalize(nj+math.tan(JITANG*math.pi/180)*torch.nn.functional.normalize(g,dim=0),dim=0)
            djit=(float(torch.rand(1))*2-1)*JIT*float(st["hc"])   # +/- JIT cells along the normal
            img,af,_,_=render(t["mvp"],nj,t["d"]+djit,grad=True)
            m=(af[:1]>0).float()
        else:
    ...
    (loss/len(T)+reg+NORMTV*ntv/len(T)).backward(); opt.step(); opt.zero_grad(set_to_none=True)
    if LNORM>0:
        with torch.no_grad():
            cur=di.feat.norm(dim=1); scale=(LNORM*norm0.clamp_min(1e-6)/cur.clamp_min(1e-6)).clamp(max=1.0)
            di.feat.mul_(scale[:,None])
    if step%50==0 or step==1:
        print(f"  step {step:4d}  face {float(loss)/len(T):.4f}  anchor {float(reg):.4f}  ntv {float(ntv)/len(T) if NORMTV>0 else 0:.4f}  {time.time()-t0:.0f}s",flush=True)
