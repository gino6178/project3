...
for step in range(1, STEPS + 1):
    ...
    if step % 2000 == 0 or step == STEPS:
        torch.save({"model": model.state_dict(), "mean": D["mean"], "std": D["std"],
                    "crop": CROP, "step": step}, f"{W}/latdiff_{OBJ}.pt")
print(f"latdiff_{OBJ}.pt written, {time.time()-t0:.0f}s")
