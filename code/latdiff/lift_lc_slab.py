...
THK=float(os.environ.get("THK","2.82")); NSUB=int(os.environ.get("NSUB","5"))
TAG=os.environ.get("TAG","mt10"); LR=float(os.environ.get("LR","5e-3")); ANCHOR=float(os.environ.get("ANCHOR","1.0"))
...
T=torch.load(f"{W}/targets_lc_{OBJ}.pt", map_location=dev)
for t in T:
...
with torch.no_grad():
    for t in T:
        if not t["name"].endswith("_h"):    # supervised
            ...
            img,_,_,_=ON.render_section(st,glctx,t["mvp"],t["n"],t["d"],S,exterior=True,thickness=THK,n_sub=NSUB)
...
def render(mvp,n,d,grad=False):
    ...
    with ctx: return ON.render_section(st,glctx,mvp,n,float(d),S,exterior=True,thickness=THK,n_sub=NSUB)
