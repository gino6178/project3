...
T=torch.load(f"{W}/targets_denselc4_{OBJ}.pt", map_location=dev)
for t in T:
...
nsup=sum(1 for t in T if t.get("sup")); nhld=len(T)-nsup
print(f"{len(T)} targets: {nsup} supervised frozen, {nhld} dense match+SDEdit (all globally recoloured)")
...
out=f"{W}/s_v2_mt14_{OBJ}"; os.makedirs(out,exist_ok=True)
