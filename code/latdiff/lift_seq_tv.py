...
STEPS=int(os.environ.get("STEPS","500")); TV=float(os.environ.get("TV","1.0")); LR=float(os.environ.get("LR","5e-3")); ANCHOR=float(os.environ.get("ANCHOR","1.0"))
...
T=torch.load(f"{W}/targets_seq_{OBJ}.pt", map_location=dev)
for t in T:
...
# TV neighbour structure for smoothing out the warp's fine grid artifact
_dims=[int(solid[:,i].max())+2 for i in range(3)]
_lin=(solid[:,0]*_dims[1]+solid[:,1])*_dims[2]+solid[:,2]
_cellat=torch.full((_dims[0]*_dims[1]*_dims[2],),-1,dtype=torch.long,device=dev); _cellat[_lin]=torch.arange(len(solid),device=dev)
_str=torch.tensor([_dims[1]*_dims[2],_dims[2],1],device=dev)
def tv_loss(rgb):
    loss=0.0
    for ax in range(3):
        nb=_cellat[(_lin+_str[ax]).clamp(max=len(_cellat)-1)]; ok=nb>=0
...
for step in range(1,STEPS+1):
    ...
    reg=ANCHOR*((di()-rgb0)**2).mean()+TV*tv_loss(di())
...
out=f"{W}/s_v2_mt5_{OBJ}"; os.makedirs(out,exist_ok=True)
