"""The successful sample_mask recipe, with each plane's own silhouette as the mask.

sample_mask succeeded because it constrained the outline and let the model generate everything
inside from noise -- so the model placed the peel and the columella where its learned distribution
wants them, not where a weak render put them. Masked SDEdit failed because (a) the flesh mask
excluded the peel, pinning it to the render, and (b) SDEdit from the render's weak axis never
strengthened it.

This uses each plane's FULL silhouette (flesh + peel, from exterior=True) as the mask, and
generates inside it from noise (RePaint: outside pinned to white at every step). So the peel ring
is inside the region the model paints, and the columella is generated, not inherited.
"""
import os, sys
...
CKV = os.environ.get("CKV", "/workspace/sindiff/OUTPUT/sd-vlong/model002000.pt")
CKT = os.environ.get("CKT", "/workspace/sindiff/OUTPUT/sd-vtrans/model002000.pt")
mL, diff = load(CKV); mT, _ = load(CKT)
def arr(t): return (t.clamp(0,1).permute(1,2,0).cpu().numpy()*255).astype(np.uint8)

@torch.no_grad()
def maskgen(m, mask):
    """RePaint: generate inside the mask from noise, pin outside to white each step."""
    known = torch.ones(1, 3, S, S, device=dev)          # white background in [-1,1]
...
@torch.no_grad()
def silhouette(mvp, n, d):
    # full cross-section outline: flesh (exterior=False alpha) OR shell. Use exterior=True alpha.
    _, af, _, _ = ON.render_section(st, glctx, mvp, n, float(d), S, exterior=True)
    return (af[:1] > 0).float()[None]

def lab(txt, wd):
    im=Image.new("RGB",(wd,22),(255,255,255));dr_=ImageDraw.Draw(im)
    try:f=ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf",14)
    except:f=ImageFont.load_default()
    dr_.text((6,3),txt,fill=(30,30,30),font=f);return np.asarray(im)

rows=[]
for name, P, M, model in (("LONG held-out", C["ev_planes"], C["ev_mvp"], mL),
                          ("TRANS held-out", C["eh_planes"], C["eh_mvp"], mT)):
    cells=[]
    for k in range(min(6,len(P))):
        ...
        mask = silhouette(mvp,n,float(P[k,3]))
        gen = maskgen(model, mask)
        cells.append(np.concatenate([arr(img), np.full((S,4,3),255,np.uint8), gen],1))
    row = np.concatenate(cells,1)
    rows.append(np.concatenate([lab(f"{name}: render | full-silhouette mask-gen", row.shape[1]), row],0))
    print(f"{name} done", flush=True)
Image.fromarray(np.concatenate(rows,0)).save("/workspace/maskgen.png")
print("done")
