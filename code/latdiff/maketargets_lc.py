...
def rgb2ycbcr(x):  # x in [0,1], shape 1,3,H,W
    r,g,b=x[:,0],x[:,1],x[:,2]
...
def ycbcr2rgb(x):
    y,cb,cr=x[:,0],x[:,1],x[:,2]; cb=cb-0.5; cr=cr-0.5
    r=y+1.402*cr; g=y-0.344136*cb-0.714136*cr; b=y+1.772*cb
    return torch.stack([r,g,b],1).clamp(0,1)
def struct_from_gen_color_from_render(gen01, render01):
    "luma from the SinDiffusion output (structure), chroma from the render (per-depth colour)."
...
for name,P,M,model_cur,photo,psil in groups:
    for k in range(len(P)):
        ...
        with torch.no_grad():
            img,af,_,_=ON.render_section(st,glctx,mvp,n,float(P[k,3]),S,exterior=True)
        ...
        gen01=(out*0.5+0.5).clamp(0,1)
        render01=(img[None]).clamp(0,1)
        swapped=struct_from_gen_color_from_render(gen01,render01)
        out=(mask*swapped+(1-mask)*white)*2-1
        targets.append(dict(name=name,k=k,n=n.cpu(),d=float(P[k,3]),
                            mvp=mvp.cpu(),tgt=out.cpu().half(),mask=mask.cpu().bool()))
    print(f"  {name}: done ({time.time()-t0:.0f}s)",flush=True)
torch.save(targets, f"{W}/targets_lc_{OBJ}.pt")
