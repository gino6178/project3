"""Masked generation with RePaint resampling -- the harmonisation step the naive mask dropped.

Free generation produces the columella; the naive mask (pin outside to white every step) does not,
because the injected white boundary biases the interior denoising and there is no step that lets
inside and outside agree. RePaint (Lugmayr et al. 2022) fixes this by resampling: after each
denoise, jump back up one step by re-adding noise and denoise again, U times, so the generated
interior and the known exterior are harmonised before moving on. That is the missing piece.
"""
import os, sys
...
U = int(os.environ.get("U", "5"))     # resampling repeats per step
...
d.update(image_size=256,num_channels=64,num_head_channels=16,channel_mult="1,2,4",
         ...
         noise_schedule="linear",learn_sigma=False,class_cond=False,timestep_respacing="250")
...
@torch.no_grad()
def repaint(mask, seed):
    torch.manual_seed(seed)
    known = torch.ones(1,3,S,S,device=dev)
    x = torch.randn(1,3,S,S,device=dev)
    T = diff.num_timesteps
    for i in reversed(range(T)):
        t = torch.full((1,),i,device=dev,dtype=torch.long)
        for u in range(U):
            # known region at level i, unknown from current x
            x = (1-mask)*diff.q_sample(known,t) + mask*x
            x = diff.p_sample(model, x, t, clip_denoised=True, model_kwargs={})["sample"]
            if u < U-1 and i > 0:
                # jump back up one step: re-add noise to return to level i (harmonise)
                beta = 1 - ab[i]/ab[i-1]
...
rn=np.concatenate([arr(img)]+[naive(mask,s) for s in range(3)],1)
rr=np.concatenate([arr(img)]+[repaint(mask,s) for s in range(3)],1)
Image.fromarray(np.concatenate([lab("naive mask: render | 3 samples",rn.shape[1]),rn,
                                lab(f"RePaint resample U={U}: render | 3 samples",rr.shape[1]),rr],0)).save("/workspace/repaint.png")
print("done")
