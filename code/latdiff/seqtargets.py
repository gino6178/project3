"""Dense targets by sequential propagation: mask-gen the first plane, then carry it across depth.

Each plane needs its own realistic interior (columella, peel) but adjacent planes must agree or the
lift averages them to mush. So the first plane is generated from noise inside its silhouette
(mask-gen -- real structure, some variety), and every next plane takes the previous plane's output,
warps it into the new silhouette, and SDEdits it lightly: the structure carries over and only adapts
to the changing shape. This is the inter-slice trajectory alignment the medical-imaging papers use,
so the dense sweep is both sharp and consistent. Supervised planes are added as render targets for
the lift to anchor.
"""
import os, sys, time
...
STR=float(os.environ.get("STR","0.35")); SPACING=float(os.environ.get("SPACING","2.5"))
...
def warp_to(src, srcmask, dstmask):
    px0,py0,px1,py1=bbox(srcmask[0,0]>0.5); rx0,ry0,rx1,ry1=bbox(dstmask[0,0]>0.5)
    ...
    return F.grid_sample(src,grid,align_corners=True,padding_mode="border")
model_cur=None
@torch.no_grad()
def maskgen(mask,seed):
    ...
    for i in reversed(range(diff.num_timesteps)):
        ...
        x=mask*x+(1-mask)*diff.q_sample(white,t)
...
k=len(C["v_mvp"])//2
fams=[("long",  torch.as_tensor(C["v_mvp"][k],dtype=torch.float32,device=dev).contiguous(), C["v_planes"][k,:3], mL),
      ("trans", torch.as_tensor(C["h_mvp"],dtype=torch.float32,device=dev).contiguous(), C["h_planes"][0,:3], mT)]
targets=[]; t0=time.time()
for name,mvp,nvec,model in fams:
    model_cur=model
    n=torch.as_tensor((nvec/np.linalg.norm(nvec)).astype(np.float32),device=dev)
    pr=(cen@n); d0=float(-pr.max())+3*hc; d1=float(-pr.min())-3*hc
    ND=int(abs(d1-d0)/(SPACING*hc)); ds=np.linspace(d0,d1,ND)
    prev=None; prevmask=None
    # start from the middle (biggest silhouette) and go both ways for stable propagation
    order=sorted(range(ND),key=lambda i:abs(i-ND//2))
    outputs={}
    for oi,idx in enumerate(order):
        dd=float(ds[idx])
        with torch.no_grad():
            _,af,_,_=ON.render_section(st,glctx,mvp,n,dd,S,exterior=True)
        mask=(af[:1]>0).float()[None]
        if float(mask.sum())<100: continue
        if oi==0:
            out=maskgen(mask,seed=0)
        else:
            # nearest already-done neighbour as the source
            done=[j for j in outputs]
            src_idx=min(done,key=lambda j:abs(j-idx))
            src,srcmask=outputs[src_idx]
            warped=warp_to(src,srcmask,mask)
            out=sdedit(warped*2-1,mask,STR)*0.5+0.5
            out=(mask*out+(1-mask)*white)
        outputs[idx]=(out.detach(),mask.detach())
        targets.append(dict(name=name,n=n.cpu(),d=dd,mvp=mvp.cpu(),tgt=(out*2-1).cpu().half(),mask=mask.cpu().bool()))
    print(f"  {name}: {len(outputs)} propagated planes, {time.time()-t0:.0f}s",flush=True)
...
torch.save(targets,f"{W}/targets_seq_{OBJ}.pt")
nd=sum(1 for t in targets if not t.get("sup"))
print(f"saved {len(targets)}: {nd} propagated, {len(targets)-nd} supervised, {time.time()-t0:.0f}s")
