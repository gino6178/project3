...
Features, in order of preference:
  ...
  * otherwise the decoded colour in CIELAB, which is at least perceptually uniform, together
    ...
    python code/evaluate/material_segment.py MODEL.ply LATTICE_DIR OUT.png [K] [ANCHOR.pt]
"""
import os
import sys
...
sys.path += [_HERE, os.path.join(_CODE, "src"), os.path.join(_CODE, "inherited"),
             _FN_ROOT, os.environ.get("GS_ROOT", _FN_ROOT + "/gaussian-splatting")]
os.chdir(_FN_ROOT)
...
def main(ply, lat_dir, out, k=4, anchor_pt=None):
    ...
    torch.save(lab.cpu(), base + "_labels.pt")
    # material.py reads a .npy; writing both is what lets the two run back to back
    np.save(base + "_labels.npy", lab.cpu().numpy())
