...
_shell_tgt, _shell_msk, _shell_geo = [None], [None], [None]
_AXES_CACHE = [None]
...
if __name__ == "__main__":
    ...
    def density_and_prune(e=0):
        ...
        if ANCHOR and SHELL_PIN:
            ...
            if _shell_tgt[0] is None:
                ...
                # What the exterior is, taken from the occupancy rather than from the level.
                #
                # `cell_level` is a resolution, not a surface. voxelize.py assigns it by
                # `r/R > skin_frac`, which is a sphere, and the objects are not spheres: the
                # orange's own surface runs from r 0.643 to 0.798 while the sphere sits at
                # 0.711, so 41.8% of the outermost cells are labelled interior. Nothing then
                # pins them and the sections repaint them -- measured over training, the
                # surface cells the label got right move 0.021 in RGB and the ones it missed
                # move 0.464, from peel [0.867 0.451 0.086] to pith [0.970 0.745 0.341]. That
                # is the pale mottling on the peel, and it is why closing the shell barely
                # helped: the holes were never the problem, the labelling was.
                #
                # A cell is on the surface when the shape says so, not when a radius does.
                # occupancy.surface_cells is the one definition; skin_project paints by the same
                # call, so the two routes cannot drift apart again.
                from occupancy import surface_cells
                _lat = torch.load(os.path.join(os.path.dirname(args.gs_path), "lattice.pt"))
                _p = gaussians.get_xyz.detach()[:_lv.shape[0] * _K:_K]
                _k = int(_os.environ.get("SHELL_PIN_LAYERS", "2"))
                _surf = surface_cells(_p, float(_lat["coarse_dx"]), layers=_k)
                _msk = (_lv != 0) | _surf[:_lv.shape[0]]
                if DDP_RANK == 0:
                    print(f"  exterior: {int((_lv != 0).sum()):,} cells by level, "
                          f"{int(_surf.sum()):,} on the occupancy boundary, "
                          f"{int(_msk.sum()):,} pinned "
                          f"({int((_surf[:_lv.shape[0]] & (_lv == 0)).sum()):,} the level missed)")
                _shell_msk[0] = _msk.repeat_interleave(_K)[:gaussians.get_xyz.shape[0]]
                _g0 = GaussianModel(0)
                _g0.load_ply_zero_sh(args.gs_path)
                _shell_tgt[0] = (_g0._features_dc.detach().squeeze(1) * 0.28209479177387814
                                 + 0.5).clamp(0, 1)
                # The geometry to hold, from the same file as the colour rather than from the
                # decoder's first guess at it -- see AnchorDecoder.freeze_geometry.
                _shell_geo[0] = (_g0.get_xyz.detach().clone(),
                                 torch.exp(_g0._scaling.detach()).clone(),
                                 _g0._rotation.detach().clone())
                del _g0
                if DDP_RANK == 0:
                    print(f"  shell pinned to the projection: "
                          f"{int(_shell_msk[0].sum()):,} primitives")
            _dec.pin_colour(_shell_tgt[0], _shell_msk[0])
            if SHELL_PIN_GEOM and _dec.geom_frozen is None:
                ...
                _n = _dec.freeze_geometry(_shell_msk[0], src=_shell_geo[0])
                if DDP_RANK == 0:
    ...
    # Alternating families, the way a GAN alternates its two objectives. Both passes run in
    # every iteration by default, longitudinal first and transverse second, which means the last
    # gradient of every iteration is always the transverse one and that family is systematically
    # favoured. It shows: the baseline scores 86.4 transversally against 147.6 longitudinally, a
    # ratio of 1.71, and the route-2 orange 66.5 against 247.3, a ratio of 3.72 -- worse where the
    # interior starts as flat grey and there is further to move, so whichever family is applied
    # last takes more of the distance.
    #
    # With SEC_ALTERNATE each iteration runs one family, and which one alternates, so neither is
    # ever the one that finishes. It is done by emptying the other family's plane range rather
    # than by branching around the two blocks: the setup either block does -- a camera, a
    # preprocess -- is cheap and harmless, and re-indenting three hundred lines to save it is the
    # kind of edit that breaks something else.
    SEC_ALTERNATE = int(_os.environ.get("SEC_ALTERNATE", "0"))
    _v_cache = []
    for j in range(ABL_ITERS):
        _do_v = (not SEC_ALTERNATE) or (j % 2 == 0)
        _do_h = (not SEC_ALTERNATE) or (j % 2 == 1)
        density_and_prune(j)
        print(f"Starting iteration {j}")
        # One iteration's longitudinal targets, kept so the transverse pass can be
        # reconciled against them where the planes meet. Under alternation the transverse
        # iteration has none of its own, so the most recent set stands rather than the pass
        # losing the thing it reconciles against.
        if _do_v:
            _v_cache = []
        for i in (range(DDP_RANK, N_VPLANES, DDP_WORLD) if _do_v else ()):
        ...
        for i, c in (list(enumerate(centers[H_LO:H_HI]))[DDP_RANK::DDP_WORLD] if _do_h else []):
            ...
            if SEC_PATCH > 0:
                ...
                total_loss = 0.7 * get_ssim_loss(rendering, ground_truth_tensor)
                total_loss += 0.3 * torch.nn.functional.mse_loss(rendering, ground_truth_tensor)
            # The transverse family's residual, which was never recorded. `_RESID` feeds the
            # convergence test that decides when a reference has stopped paying, and every
            # append to it was in the longitudinal loop -- twice, from a duplicated block whose
            # copy printed itself as `h{i}`. So the log showed the two families with identical
            # losses, which is what they were: one family, printed under both names. The
            # schedule those numbers drive is off in every run here (REF_CONV_TAU=0 makes
            # `converged_enough` return False on its first line), so no trained model is
            # affected -- but anyone turning it on would have been steering by half the planes
            # counted twice.
            with torch.no_grad():
                _res = float(torch.nn.functional.mse_loss(rendering, ground_truth_tensor))
            _RESID.append(_res)
