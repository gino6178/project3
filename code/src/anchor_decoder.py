...
# The nearest-neighbour self-match, dropped by index rather than by how small the distance
# looks. 0 restores the float32 test that could not see it, which is only useful for measuring
# what it cost -- see install().
CAP_FIX = os.environ.get("ANCHOR_CAP_FIX", "1") == "1"
...
class AnchorDecoder(nn.Module):
    ...
    def forward(self, cam_center=None):
        ...
        if self.col_pin is not None:
            # The colour held the way the geometry below is held, by overwriting rather than by
            # a stored difference. `set_colour` records `target - decode` once and adds it back
            # on every later decode, so it is exact only until the next optimiser step: over one
            # epoch the shell's coefficients drifted a mean of 0.156 and a maximum of 3.66 away
            # from the projection, and the model is written at the end of an epoch, so that
            # drift is what got saved. It shows as a red arc along a section plane's footprint
            # on the peel -- 369 pixels of one view, colour [0.978 0.636 0.136] in the ply
            # against [0.972 0.402 0.059] in the render. Overwriting removes it exactly: 0 such
            # pixels, and the whole view's distance to the ply falls from 0.0374 to 0.0194.
            #
            # It also stops the gradient at the pinned cells, which is what SHELL_PIN says it
            # does -- the residual form let the sections keep pushing a shell that was going to
            # be reset anyway.
            _m, _t = self.col_pin
            rgb = torch.where(_m[:, None], _t, rgb)
    ...
    def freeze_geometry(self, mask, src=None):
        """Reproduce the shell's geometry on every later decode.

        `src` is (xyz, scale, rot) from the model the exterior came from. Without it the geometry
        held is whatever the decoder happened to produce on its first decode, which is not the
        same thing and this function's own name says so: the decoder's scale head starts at
        `cap * sigmoid(ANCHOR_SCALE_BIAS)` and reproduces nothing, so the shell rendered at sigma
        0.005081 where the ply wrote 0.004263. Colour alone was then exact and the render still
        differed from the ply's by 0.0177 per pixel; holding the ply's own scale and rotation as
        well takes that to 0.0088.
        """
        with torch.no_grad():
            xyz, _, _, scale, rot = self()
            if src is not None:
                _x, _s, _r = (v.to(xyz.device, xyz.dtype) for v in src)
                if _x.shape[0] != xyz.shape[0]:
                    _x, _s, _r = (v.repeat_interleave(self.K, dim=0) for v in (_x, _s, _r))
                n = min(xyz.shape[0], _x.shape[0])
                xyz, scale, rot = xyz.clone(), scale.clone(), rot.clone()
                xyz[:n], scale[:n] = _x[:n], _s[:n]
                rot[:n] = torch.nn.functional.normalize(_r[:n], dim=1)
    ...
    def pin_colour(self, target, mask):
        """Hold `mask`'s colour at `target` on every decode, exactly, taking no gradient.

        Unlike `set_colour` this is not a residual and does not need re-applying: what it stores
        is the answer, not the difference between the answer and a decoder that keeps moving.
        """
        with torch.no_grad():
            n = self.anchor_xyz.shape[0] * self.K
            t = target.to(self.feat.device, self.feat.dtype)
            if t.shape[0] != n:
                t = t.repeat_interleave(self.K, dim=0)
            m = mask.reshape(-1).to(self.feat.device)
            if m.shape[0] != n:
                m = m.repeat_interleave(self.K)
            self.col_pin = (m[:n].clone(), t[:n].clone())
        return int(self.col_pin[0].sum())
...
def install(gaussians, level_path, lattice_path, K=4, f_dim=32, view_dependent=False):
    ...
    #
    # The self-match is dropped by index, not by how small the distance looks. `torch.cdist`
    # in float32 evaluates |a|^2 + |b|^2 - 2a.b, so a point against itself comes back as the
    # rounding error of that cancellation -- 1e-4 to 1e-3 at these coordinates, not zero --
    # and a `< 1e-9` test does not see it. 15.9% of the orange's anchors then took their own
    # rounding error as their nearest neighbour and got a cap 24x too small; their children
    # decode at sigma 0.000357 into a cell 0.005900 wide, 17x under, and SHELL_PIN_GEOM
    # freezes that at iteration zero so it never recovers. The skin stops covering itself and
    # the white background reads through it -- with the colour still exactly the ply's, which
    # is what made it look like an appearance bug.
    with torch.no_grad():
        nnd = torch.empty(anchors.shape[0], device=dev)
        rows = torch.arange(4000, device=dev)
        for s0 in range(0, anchors.shape[0], 4000):
            ...
            for j in range(0, anchors.shape[0], 200000):
                ...
                if CAP_FIX:
                    ...
                    dm[dm < 1e-9] = 1e9          # the old test, kept for A/B only
                best = torch.minimum(best, dm.min(1).values)
                del dm
            nnd[s0:s0 + 4000] = best
        # Two anchors can occupy the same point. voxelize.py rounds the coarse set and the fine
        # set onto their own grids from a shared origin, and which set a primitive joins is
        # decided by its radius, not its position -- so at the skin boundary a coarse cell and a
        # fine cell land on the same coordinate. 7,330 of the orange's anchors have a distinct
        # neighbour at distance zero. The old threshold hid them along with the self-match; read
        # honestly there is no spacing to take, and a cap of zero decodes to a splat of zero. The
        # anchor's own cell is what the cap would have been without a neighbour to shorten it.
        if CAP_FIX:
            nnd = torch.where(nnd < cells * 0.1, cells, nnd)
